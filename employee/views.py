    ...
    if request.method == 'POST':
        ...
        if password1 == password2:
            if employeedetails.objects.filter(employee_email=email1,employee_id=empid1).exists():
                ...
                form=employeedetails(employee_id=empid1, employee_name=empname1, employee_email=email1, employee_password=password1)
                try:
                    form.full_clean()
                    form.save()
...
def login(request):
    if 'user' in request.session:
        ...
        if request.method=='POST':
            ...
            if employeedetails.objects.filter(employee_id=us,employee_password=pw).exists():
                emp=employeedetails.objects.get(employee_id=us,employee_password=pw)
                request.session['user']={'id':us, 'psw':pw}
